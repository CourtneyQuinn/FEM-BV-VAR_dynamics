...
from ..utils.validation import check_array_shape
...
class FEMBVVARX:
    ...
    def fit(self, X, y=None, exog=None): # pylint: disable=unused-argument
    ...
    def fit_predict(self, X, y=None, exog=None): # pylint: disable=unused-argument
        ...
        self._max_tv_norm = max_tv_norm # pylint: disable=attribute-defined-outside-init
        ...
        min_cost = np.infty
        self.converged_ = False # pylint: disable=attribute-defined-outside-init

        random_state = check_random_state(self.random_state)

        for init in range(n_init):
            self._init_start_time = time.perf_counter() # pylint: disable=attribute-defined-outside-init
            self._print_verbose_msg_init_beg(init)

            if do_init:
                self._initialize(X, random_state, exog=exog)

            self._fembv_weights = FEMBVWeights( # pylint: disable=attribute-defined-outside-init
                self.weights_, max_tv_norm=self._max_tv_norm,
                weights_solver_kwargs=self.weights_solver_kwargs)

            cost = (np.infty if do_init else self.cost_) # pylint: disable=access-member-before-definition
            converged = False

            for n_iter in range(self.max_iter):

                self._iter_start_time = time.perf_counter() # pylint: disable=attribute-defined-outside-init
                ...
                self._iter_end_time = time.perf_counter() # pylint: disable=attribute-defined-outside-init
                ...
                if abs(change) < self.tol:
                    converged = True # pylint: disable=attribute-defined-outside-init
                    break

            self._init_end_time = time.perf_counter() # pylint: disable=attribute-defined-outside-init
            ...
            if not converged:
                ...
                self.converged_ = converged  # pylint: disable=attribute-defined-outside-init

        self.weights_ = best_weights # pylint: disable=attribute-defined-outside-init
        self._set_parameters(best_params)
        self.n_iter_ = best_n_iter # pylint: disable=attribute-defined-outside-init
        self.cost_ = min_cost # pylint: disable=attribute-defined-outside-init
        self.log_likelihood_ = self._log_likelihood(X, exog=exog) # pylint: disable=attribute-defined-outside-init
        self.rmse_ = self._rmse(X, exog=exog) # pylint: disable=attribute-defined-outside-init

        return self.weights_

    def predict(self, X, y=None, exog=None): # pylint: disable=unused-argument
        ...
        for init in range(n_init):
            ...
            self.weights_ = weights.copy()  # pylint: disable=attribute-defined-outside-init
        ...
        self._max_tv_norm = old_max_tv_norm # pylint: disable=attribute-defined-outside-init
    ...
    def _check_initial_parameters(self, X, exog=None):
        ...
        self.max_order_ = np.max(self.orders)  # pylint: disable=attribute-defined-outside-init
    ...
    def _stack_predictors(self, X, exog=None):
        ...
        self._Y_k = {k: None for k in range(self.n_components)}  # pylint: disable=attribute-defined-outside-init
        self._Theta_k = {k : None for k in range(self.n_components)}  # pylint: disable=attribute-defined-outside-init
    ...
    def _initialize(self, X, random_state, exog=None):
        ...
        self.weights_ = (weights if self.weights_init is None  # pylint: disable=attribute-defined-outside-init
    ...
    def _initialize_parameters(self, X, weights, exog=None):
        ...
        self.means_ = means if self.means_init is None else self.means_init  # pylint: disable=attribute-defined-outside-init
        self.endog_coef_ = (endog_coef if self.endog_coef_init is None  # pylint: disable=attribute-defined-outside-init
                            else self.endog_coef_init)
        self.exog_coef_ = (exog_coef if self.exog_coef_init is None  # pylint: disable=attribute-defined-outside-init
        ...
        if self.precisions_init is None:
            self.covariances_ = covariances  # pylint: disable=attribute-defined-outside-init
            self.precisions_cholesky_ = _compute_precision_cholesky(covariances)  # pylint: disable=attribute-defined-outside-init
        else:
            self.precisions_cholesky_ = np.array(  # pylint: disable=attribute-defined-outside-init
                [sl.cholesky(prec_init, lower=True)
                 for prec_init in self.precisions_init])
            self.covariances_ = _compute_covariances(  # pylint: disable=attribute-defined-outside-init
    ...
    def _update_parameters(self, X, weights, exog=None):
        ...
        self._Theta_k, self.covariances_ = self._estimate_varx_parameters(  # pylint: disable=attribute-defined-outside-init
            X, weights, self.reg_covar)

        self.means_, self.endog_coef_, self.exog_coef_ = \
            self._unpack_flattened_parameters(X, self._Theta_k, exog=exog)  # pylint: disable=attribute-defined-outside-init

        # compute precision matrix via Cholesky decomposition
        self.precisions_cholesky_ = _compute_precision_cholesky(self.covariances_)  # pylint: disable=attribute-defined-outside-init
    ...
    def _update_weights(self, X):
        ...
        self.weights_ = self._fembv_weights.weights  # pylint: disable=attribute-defined-outside-init
    ...
    def _set_parameters(self, params):
        ...
        (self.means_, self.endog_coef_, self.exog_coef_,  # pylint: disable=attribute-defined-outside-init
         self.covariances_, self.precisions_cholesky_) = params  # pylint: disable=attribute-defined-outside-init
        ...
        self.precisions_ = np.empty(self.precisions_cholesky_.shape)  # pylint: disable=attribute-defined-outside-init
        for k, prec_chol in enumerate(self.precisions_cholesky_):
            self.precisions_[k] = np.dot(prec_chol, prec_chol.T)  # pylint: disable=attribute-defined-outside-init
