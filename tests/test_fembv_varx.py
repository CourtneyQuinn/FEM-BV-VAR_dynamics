...
from clustering_dynamics.models.fembv_varx import (_compute_precision_cholesky,
...
def test_throws_when_given_invalid_n_components():
    """Test fitting throws exception when given invalid number of components."""
...
def test_rmse_correctly_calculated():
    ...
    assert np.abs(expected_rmse - calculated_rmse) < 1e-3
...
def test_linear_varx_leastsq_example_1():
    ...
    assert np.allclose(df_bias * model.covariances_, expected_Sigma_LS, atol=1e-5)
...
def test_finds_correct_weights_with_hidden_ar1_process_different_levels():
    ...
    if model._max_tv_norm is not None:
        assert np.all(
            np.sum(np.abs(np.diff(model.weights_, axis=0)), axis=0) < model._max_tv_norm)
...
def test_predicts_correct_weights_with_hidden_step_independent_noise():
    ...
    predicted_weights, predicted_cost, predicted_log_like, predicted_rmse = model.predict(X_test)
    ...
    for t in range(n_test_samples):
        expected_log_like += ss.multivariate_normal.logpdf(
            X_test[t], mean=model.means_[state_index], cov=model.covariances_[state_index])
...
def test_predicts_correct_weights_with_hidden_ar1_process_different_levels():
    ...
    predicted_weights, predicted_cost, predicted_log_like, _ = model.predict(X_test)
    ...
    expected_log_like = 0.0
    for t in range(1, n_test_samples):
        X_hat = model.means_[state_index] + np.dot(model.endog_coef_[state_index, 0], X_test[t - 1])
        expected_log_like += ss.multivariate_normal.logpdf(
